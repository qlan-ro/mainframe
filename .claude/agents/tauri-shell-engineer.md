...
3. **C1 — login-shell environment (non-negotiable):** before spawning the sidecar, capture the user's login-shell env (`$SHELL -lic env`, default `/bin/zsh`, ~5s timeout, parse KEY=VALUE) and pass it to the sidecar, mirroring `packages/app-electron/src/main/index.ts:resolveShellEnv()` + `startDaemon()`. Prefer the `fix-path-env` crate. Replicate the fallback (`~/.local/bin:/usr/local/bin:/opt/homebrew/bin:/opt/homebrew/sbin`). The daemon's own `enrichPath()` is a secondary net, not the primary.
