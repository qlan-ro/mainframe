...
- **Mainframe queue implementation** (updated 2026-07-04): the daemon-held queue era (`ChatManager.chatQueues`, 2026-06-27 → 2026-07-04) is REVERTED — both `main` and `feat/app-tauri-wt` now forward straight to stdin and mirror CLI state via `queuedRefs`, so mid-turn drain and batching apply. `cancel_async_message` IS implemented in the CLI (`print.ts:3011`, `dequeueAllMatching` by uuid) — the refactor's "never implemented" rationale was wrong; see memory `daemon-owns-queued-messages`.
