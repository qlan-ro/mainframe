...
- **`/` skills:** `<Unstable_TriggerPopover char="/" adapter={skillsAdapter}>` + `.Directive` (see note below). `skillsAdapter` (`buildSkillsTriggerAdapter(skills)`): one "Skills" category; `categoryItems`/`search` map `Skill → Unstable_TriggerItem { id: invocationName ?? name, type:'skill', label: displayName||name, description }` and filter in-memory. On select, insert `/<invocationName ?? name> `.

  > **Impl correction (post-build):** both triggers use **`.Directive`** with the literal formatter, NOT `.Action`. `.Action` + `removeOnExecute:true` *deletes* the inserted text after firing — the opposite of what we want (`/skill` must stay in the message). `.Directive` + the literal formatter is the correct primitive for "insert plain text and keep it".
