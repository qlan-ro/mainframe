...
> **STATUS — 2026-06-17: Phase 2 (per-site sweeps) IMPLEMENTED & verified** (commits `e3eb8503`, `0264f4b8`).
> - **Selection tint** (`bg-accent` hover-tint reused for selected → `bg-mf-selection`): ProviderModelSelect active model row, SettingsSidebar active nav + provider sub-item, DirectoryPickerModal selected dir, ReviewFileTree selected file, FindInPathModal cursor row. 2 new regression tests.
> - **Warm content-surface sweep:** CodeHeader two-tone header (`content2` over `code-bg`), markdown table header + stripe (`content2`), ReviewPanel two-tone split (`bg-card`), transparent surface headers (FilesTabStrip/ChatCardHeader), UnsupportedViewer card (`bg-card`), CodeRefCard body (`code-bg`).
> - **Hardcoded → tokens:** image/svg viewer checkerboard now uses the warm `--mf-viewer-check-a/b` tokens (18px tile), leveraging the Phase-0 `@theme` mapping.
> - **Deferred to Phase 3 (deliberate):** tasks priority/status pill colors (`bg-orange-100`/`bg-blue-500`) need new semantic priority/status tokens (a token-system addition, not a swap); `RunSurface` surface-header bg + `PreviewUrlBar` `bg-green-500` live in the uncommitted preview/run WIP (left for that branch); the structural viewer refactors (SvgViewer white card / Preview-Source toggle in the ViewerShell header, CsvViewer filter in the header, UnsupportedViewer chip+CTA, tab-strip 36px height) are per-surface majors. Verified: viewers+review+layout 174 green, chat parts/thread/messages 125 green, typecheck clean.
