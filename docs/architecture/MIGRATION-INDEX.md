...
## Resume here (session snapshot — 2026-06-07, end of SESSIONS-SIDEBAR session)

**Phase: the CHAT and SESSIONS surfaces are both fully built and design-conformed. Everything else (editor · terminal · settings · modals · tasks/git · plugins · sandbox/run · shell/layout chrome · packaging) is still to port.** The architectural foundations are locked and landed: the chat-runtime decision (ExternalStore + per-chat controller/reducer/projection, no message cache, refetch-on-gap), assistant-ui pinned at **react@0.14.14 / core@0.2.10 / store@0.2.13**, the shadcn `components/ui/` foundation (18 primitives) + warm-chrome theme tokens, and the C1 sidecar/login-shell-env spike.

**Landed since the prior (COMPOSER + REVIEWS) snapshot:**
- **The full review batch is resolved** — `noUncheckedIndexedAccess` on (tsconfig extends base), all endpoints Zod-validated, malformed WS frames dropped, daemon error events surfaced, unsound casts removed, failed-history-load retry banner, the `features/chat` restructure with a README charter. The 3 inline **gate cards** (`PermissionGate`/`AskUserQuestionGate`/`PlanGate`) shipped at the thread tail with count-aware optimistic reconcile (judo-A), server-authoritative config (judo-B), response-only reply seam (judo-C), and permission-reliability hardening (3s verify-timer + connected-gated ack-fallback + restore-stale guard).
- **Sessions management (build-order step 11) is BUILT + design-conformed** — one global `useRemoteThreadListRuntime` + native `ThreadListItemPrimitive` rows in a custom grouped/filtered sidebar: TIME grouping (Pinned/Today/Yesterday/Earlier) + a Sort-By menu (**project is a filter, not a group** — pills + per-row colored chip), collapsible project pills (+N more), a collapsible wrapping tag filter bar (+N more, no horizontal scroll), tag management (`TagPopover` + single `TagPopoverHost` at root, cascade-mirror, validate-tag-name), a `SessionContextMenu` (Pin/Rename/Archive/Copy/Tags), the `ArchiveWorktreeDialog` archive-confirm bridge, dense rows with hover actions, a warm-glass panel, and a thin auto-hide scrollbar.
- **Boot auto-opens the most-recent session** (parity with desktop, commit `56ddee9f`). **The new-thread flow is draft-aware** (commit `3f85617f`): the picker is skipped when a project pill is active, live adapter/model/permission/plan/effort/feature selectors run before first send, and the chat is created on first send only (no empty sessions — D3 preserved).

**Remaining = build-order step 12** (data-testid saturation + the ADR stress matrix: long chat · nested subagent + mid-turn permission · reconnect · optimistic dedup · two windows), **then the non-chat surfaces** (editor/viewers · terminal · settings · overlays/review · tasks/git · sandbox/run · plugins) which first need the **Tauri bridge (`lib/tauri/`)**, the **typed-surface layout engine + surface-intent bus**, and a **sidecar packaging** spike.
...
- **zustand is a phantom dep** — imported in 7+ `src` files (`store/unread-store`, `store/session-filters`, `sessions/runtime/*`, `tags/use-tag-popover-target`) but only resolves via `shamefully-hoist`; **must be declared in `packages/app-tauri/package.json` on merge** (a clean install / stricter hoisting breaks the build). Highest-priority merge-blocker.
...
- **SessionSidebar group-header "more" popover** is a presentational placeholder (has a testid, no menu wired).
...
8. Remaining work = build-order **step 12** (data-testid saturation + ADR stress matrix) then the non-chat **leaves**, built per the golden rule. **Start at `MIGRATION-TRACKER.md` → Consolidated Backlog + Recommended next steps (ordered)** — that's the single source of truth for what's left and in what order.
