...
**Right Inspector parity landed (2026-06-21, `14aa3838` + `8189a249`):** the right Inspector (FileTree + Changes) brought to artboard/desktop parity — `InspectorPane` now uses the **glass** card treatment (window-style-aware, symmetric with the left sidebar) instead of a solid card; the file-tree **right-click menu** gains Reveal in Finder / Copy Path / Copy Relative Path (shared `FileTreeRowMenu`); and the **Changes tab** scopes (Session / Uncommitted / Branch) now each load a distinct daemon source (new `getSessionFiles` + `getBranchDiffs` wrappers, no contract change) with auto-refresh on `context.updated` + focus. A `design-conformance` follow-up fixed a dead selected-row accent bar + icon/token/spacing nits. Full detail in `MIGRATION-TRACKER.md` (chronological log). *Not live-rendered yet — verify the glass panel in the running app.*
