...
**Remaining = the non-chat surfaces** (editor/viewers · terminal · settings · overlays/review · tasks/git · sandbox/run · plugins) plus the **sidecar packaging** spike and capabilities/CSP. The Tauri bridge core and the layout engine + intent bus are in; the editor surface is the highest-value next leaf.
