...
- ☑ **User-message states** (2026-06-10) — the three missing User-Message-States artboard variants, spec+codex-approved (`docs/superpowers/specs/2026-06-10-user-message-states-design.md`): **file-attachment pills** on the native `message.attachments` slot (`UserAttachments`, UMFileThumb visuals); **sandbox-capture context chips** (`CaptureContextRow` + `parse-captures.ts` — the sentinel is now parsed in the projection instead of rendering as raw `�MF_SANDBOX_CAPTURE` text); **render-only code-reference card** (`CodeRefCard`). Projection (`convert-message`) fills native attachments + `meta.captures`/`meta.attachmentPreviews`/`meta.codeRef`; `UserMessage` dispatches them. **codeRef has no producer yet** — render-only; the editor sender must extend the WS Zod schema + send types (additive, mobile-safe) when the editor surface lands.
