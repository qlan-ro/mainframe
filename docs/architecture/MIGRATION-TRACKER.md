...
9. ☑ **composer config toolbar** — model · permission · plan · effort · features as stateless shadcn controls, live `isRunning` disable, one optimistic-mutate helper (`dbf70ba9`, `4d9b14a1`, `f5be810b`). **NOTE:** config flows via **REST** (`PATCH /chats/:id/config` + `/tuning`), NOT `setRunConfig.custom` — the daemon exposes those routes (correct the CLAUDE.md drift). `@`-mention picker (native `Unstable_TriggerPopover`) + worktree/captures controls deferred (see #8).
