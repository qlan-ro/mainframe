...
**Verification:** `grep -rn "window\.confirm\|window\.alert\|window\.prompt" packages/ui/src packages/app-tauri/src packages/e2e` returns nothing (the brief's repo-wide criterion); `wc -l packages/ui/src/features/sessions/sidebar/SessionSidebar.tsx` is under 300; `pnpm --filter @qlan-ro/mainframe-ui exec vitest run src/features/sessions/sidebar/__tests__/SessionSidebar.test.tsx` passes.
