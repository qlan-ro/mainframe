...
- Claude's answer envelope (`docs/adapters/claude/CONSUMED-SURFACE.md`, CLAUDE-CTRL-04) requires `updatedInput` on **every** allow — omitting it makes the CLI reject the request outright, documented at `build-control-response.ts:36-39` — plus `updatedPermissions`, `message`, and a `setMode` forced to `session`.
