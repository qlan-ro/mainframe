...
> ## ⚠️ STALE — read this first
>
> **Reversed from CLI v2.1.37. Not re-verified against the installed v2.1.220,
> and not checked against the 2026-03-31 source leak.** Treat every claim here
> as "true as of v2.1.37" and re-derive anything you are about to depend on.
>
> **Provenance:** this file was lost in a history rewrite (it lived at
> `docs/PROTOCOL_REVERSED.md` as of `f6d62841`, renamed to this path at
> `75badd35`) and recovered on 2026-07-25 from unreachable git objects during
> todo #241. It is byte-identical to `git show
> 75badd35:docs/adapters/claude/PROTOCOL_REVERSED.md` apart from this banner.
>
> **Superseded in these areas** — prefer the named doc where they disagree:
>
> | Area | Authoritative doc | What changed since v2.1.37 |
> |------|-------------------|----------------------------|
> | Session JSONL format, directory layout, entry union | [`SESSIONS_JSONL.md`](SESSIONS_JSONL.md) | New `permission-mode` and `relocated` entry types; `last-prompt.leafUuid`; `agent-*.meta.json` gains `toolUseId`/`spawnDepth`; worktree transcripts relocate |
> | Permission rules, `can_use_tool`, permission modes | [`PERMISSIONS.md`](PERMISSIONS.md) | `localSettings` root moved to the canonical git repo root; `auto` joined `EXTERNAL_PERMISSION_MODES` (6 modes, not 5) |
> | Hook events and the headless hook contract | [`HOOKS.md`](HOOKS.md) | 31 hook events in 2.1.220 vs. 27 in the leak |
> | Slash-command surface and headless reachability | [`SLASH_COMMANDS.md`](SLASH_COMMANDS.md) | `thinClientDispatch` did not exist at v2.1.37 |
> | `/clear` semantics | [`CLEAR.md`](CLEAR.md) | `/clear` flipped to thin-client dispatchable (`thinClientDispatch: "post-text"`) |
>
> Its distinctive value — the `--sdk-url` **WebSocket transport**, connection
> lifecycle, and reconnection behavior — is covered by no other doc in this
> directory and is why it was restored rather than deleted.
...
### 6.15. Slash Commands via User Messages

Certain CLI commands can be sent as user messages with XML-tagged content. The CLI processes them locally and the session continues (same `session_id`).

**Commands that work via stdin:**

| Command | Effect | Re-emits `system:init`? |
|---------|--------|------------------------|
| `/clear` | Clears conversation history (full context reset) | Yes |
| `/compact` | Compresses context (keeps summary) | Yes |

**Commands that do NOT work via stdin** (terminal-only):
`/cost`, `/status`, `/config`, `/permissions`, `/mcp`, `/login`, `/logout`, `/add-dir`, `/agents`, `/doctor`, `/help`, `/terminal-setup`

**Format — XML-tagged user message:**
```json
{
  "type": "user",
  "message": {
...
**Response pattern:** The CLI emits a fresh `system:init` message followed by an `assistant` response (e.g. "Conversation cleared.") and then a `result` message.

**Built-in slash commands from `system:init`:**
The `slash_commands` field in `system:init` lists available commands. Built-in ones include: `compact`, `context`, `cost`, `init`, `pr-comments`, `release-notes`, `review`, `security-review`, `insights`. User-installed skills/plugins are also listed here.
...
**Tested modes** (CLI v2.1.37, all succeed):

| Mode | Result |
|------|--------|
| `bypassPermissions` | Success — `{ mode: "bypassPermissions" }` |
| `acceptEdits` | Success — `{ mode: "acceptEdits" }` |
| `plan` | Success — `{ mode: "plan" }` |
| `default` | Success — `{ mode: "default" }` |

> **Note:** Earlier documentation claimed `bypassPermissions` was rejected with an error. This is not the case as of CLI v2.1.37. All modes work at runtime without process restart.

**Behavior:** The CLI re-evaluates permission rules immediately. Pending `can_use_tool` requests are not re-sent — only future tool uses are affected. The mode change also triggers a `system/status` event with the new `permissionMode` field.
...
**Behavior:** Changes the model for subsequent API calls. The next `assistant` event will use the new model. Takes effect immediately without process restart.
