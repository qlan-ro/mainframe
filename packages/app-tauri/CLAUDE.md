...
- **Tool cards → MATCH (use the engine wholesale, restyle).** `MessagePrimitive.GroupedParts` + `tools.by_name`/`Fallback`; restyle the shadcn `ToolFallback`/`ToolGroup` compounds; subagent transcripts via `MessagePartPrimitive.Messages` (+ `ReadonlyThreadProvider`); per-arg streaming via `useToolArgsStatus`. Build per-family cards (Edit/Write/Bash/Read/Grep/Todo/Web/MCP/Plan/Skill/Worktree/Schedule) as `tools.by_name` entries, `ToolFallback` as the catch-all. **DROP** `makeAssistantToolUI` (deprecated) **and** the `_ToolGroup/_TaskGroup/_TaskProgress` virtual-tool encoding — use native grouping + `part.messages`. *(`groupPartByType`/`display:'standalone'` are NOT in our pinned version — supply our own `groupBy` over `buildGroupTree`.)* Task = a `tools.by_name` card whose body renders `part.messages`; TaskGroup/ToolGroup = the `GroupedParts` group-tool case; TaskProgress = a running-status slot, not a card.
