...
export function ConnectionOverlay({ open, embedded = false }: ConnectionOverlayProps): React.ReactElement | null {
  ...
  return ReactDOM.createPortal(
    <div className="fixed inset-0 z-[11000]">{body}</div>,
    document.body,
  );
}
...
function TextBlock(): React.ReactElement {
  return (
    <div className="text-center">
      <p className="text-heading font-semibold text-foreground tracking-[-0.15px]">
...
function ProgressRail(): React.ReactElement {
  return (
    <div
      className="w-[200px] h-[3px] rounded-[2px] overflow-hidden"
