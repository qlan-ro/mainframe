...
const KIND_WORD: Record<string, string> = { added: 'Added', deleted: 'Deleted', modified: 'Modified', renamed: 'Renamed' };
...
const basename = (path: string): string => (path.lastIndexOf('/') === -1 ? path : path.slice(path.lastIndexOf('/') + 1));
...
async function fetchChanges(port: number, projectId: string, chatId: string | undefined, mode: ScopeMode): Promise<FetchResult> {
  if (mode === 'session') {
...
export function ChangesPanel({ port, projectId, chatId }: ChangesPanelProps) {
  ...
  return (
    <div data-testid="changes-panel" className="flex flex-col">
      ...
      {!error && !sessionNoChat && rows === null && <div className="px-3 py-4 text-caption text-muted-foreground">Loading…</div>}
