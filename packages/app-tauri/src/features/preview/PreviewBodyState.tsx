...
export function PreviewBodyState({
  ...
  if (status === null || status === 'stopped') {
    return (
      <div
        data-testid="preview-body-stopped"
      ...
      >
        ...
        >
          ...
          <span className="font-mono text-micro text-mf-text-4">
            launches localhost:{port ?? '…'}
          </span>
        </button>
      </div>
    );
  ...
  if (status === 'starting') {
    return (
      <div
        data-testid="preview-body-starting"
        className="absolute inset-0 grid place-items-center bg-card"
      >
        <div className="flex items-center gap-[8px]">
          <Loader2 size={12} className="animate-spin text-mf-text-3" />
          <span className="text-label text-mf-text-3">
  ...
  if (status === 'running') {
    ...
    return (
      <div data-testid="preview-body-running" className="absolute inset-0">
        {device === 'desktop' ? (
          <div className={`absolute inset-0 overflow-hidden rounded-md [border:0.5px_solid_var(--border)] ${inspectFrame}`}>
  ...
  return (
    <div
      data-testid="preview-body-failed"
      className="absolute inset-0 grid place-items-center bg-card"
    >
