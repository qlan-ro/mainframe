...
vi.mock('@/features/run/format-captures', () => ({
  formatCaptures: () => ({ markdown: 'formatted captures text', attachments: [{ name: 'screenshot1.png', mediaType: 'image/png', data: 'abc123' }] }),
}));
    ...
    expect(mockSendMessage).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        content: expect.arrayContaining([
          expect.objectContaining({ type: 'text', text: 'formatted captures text' }),
        ]),
      }),
    );
  });
