...
const IDLE: DragState = { kind: null, surface: null, tabId: null, pointer: { x: 0, y: 0 }, dropZone: null };
...
export const useSurfaceDragStore = create<DragStore>((set, get) => ({
  ...
  beginSurfaceDrag(surface, e) {
    set({ ...IDLE, kind: 'surface', surface, pointer: { x: e.clientX, y: e.clientY } });
  },

  beginTabDrag(tabId, e) {
    set({ ...IDLE, kind: 'tab', tabId, pointer: { x: e.clientX, y: e.clientY } });
  },
  ...
  commit() {
    const { kind, surface, tabId, dropZone } = get();
