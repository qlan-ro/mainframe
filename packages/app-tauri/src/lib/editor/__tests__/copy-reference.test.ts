/**
 * copy-reference tests — ported verbatim from the desktop original,
 * with the Monaco import replaced by the pure lib/editor version.
 */
import { describe, expect, it } from 'vitest';
import { buildReference, findSymbolChain, buildReferenceForCm } from '../copy-reference';
...
describe('buildReferenceForCm', () => {
  it('converts 0-based CM6 line to 1-based display line', () => {
    // CM6 line 4 → display line 5
    expect(buildReferenceForCm('src/auth.ts', 4)).toBe('src/auth.ts:5');
  });

  it('includes word when provided', () => {
    expect(buildReferenceForCm('src/auth.ts', 9, 'validate')).toBe('src/auth.ts:10 (validate)');
  });

  it('falls back to "untitled" for undefined path', () => {
    expect(buildReferenceForCm(undefined, 0)).toBe('untitled:1');
  });
});
