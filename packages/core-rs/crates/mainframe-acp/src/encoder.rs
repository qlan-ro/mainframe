...
//! its stable id, not by stream position, so every text/thinking/image leaf
//! under one container id accumulates into a single item's ordered block
//! list (spec Decision 22). Invariant: that list never holds two adjacent
//! text blocks — text coalesces into the trailing text block — so the diff
//! engine's chunk appends are lossless under the client's trailing-text
//! coalescing rule.
...
/// flattened `TaskGroup`'s nested `calls`) under `container_id`. Text/image
/// leaves accumulate into one message item's ordered block list, thinking
/// leaves into one thought item, each emitted after any interleaved tool
/// calls (module doc: cross-kind order is not preserved).
fn encode_content(
    ...
    for block in content {
        match block {
            DisplayContent::Leaf(LeafContent::Text { text: t, .. }) => push_text(&mut blocks, t),
            ...
            DisplayContent::Leaf(LeafContent::SkillLoaded {
                ...
                push_text(
                    &mut blocks,
                    &format!("[skill loaded: {skill_name} ({path})]"),
                );
            }
            ...
            DisplayContent::Node(DisplayNode::Error { message }) => {
                push_text(&mut blocks, &format!("[error] {message}"));
            }
            DisplayContent::Node(DisplayNode::Compaction { .. }) => {
                push_text(&mut blocks, "[context compacted]");
            }
        }
    }
