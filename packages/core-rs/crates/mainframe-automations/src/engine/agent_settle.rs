...
impl AgentVerb {
    pub(crate) async fn settle(
        ...
        let Some(keep_going) = self.load_waiting_step(chat_id, &key).await else {
            return;
        };
        ...
        match verdict {
            Verdict::Succeed(outputs) => self.succeed_waiting_step(&key, outputs).await,
            Verdict::Fail(error) => self.fail_waiting_step(&key, keep_going, &error).await,
        }
    }
    ...
    /// Returns the step's `keepGoing` for the failure policy.
    async fn load_waiting_step(&self, chat_id: &str, key: &WaitKey) -> Option<bool> {
        ...
        Some(step.is_some_and(Step::keep_going))
    }
