...
describe('FileCredentialStore', () => {
  ...
  it('set/get round-trips a credential and persists it to disk with 0600 perms', () => {
    const store = new FileCredentialStore(filePath, silentLogger);
  ...
  it('labels() lists labels without ever exposing token values', () => {
    const store = new FileCredentialStore(filePath, silentLogger);
    store.set('gh-pat', { kind: 'token', token: 'secret123' });
    store.set('notion', { kind: 'token', token: 'other-secret' });
  ...
  it('a fresh store instance reads back a previously persisted credential', () => {
    const store1 = new FileCredentialStore(filePath, silentLogger);
    store1.set('gh-pat', { kind: 'token', token: 'secret123' });
