...
One residual, outside the six scenarios: `mock-cli` replays a uuid-carrying prompt **without**
acking when no turn is in flight (`session_trait.rs::send_message` → `queue_prompt` returns
`false`). The daemon has already enrolled that prompt in `queuedRefs`, so the ref is never
retired and later sends stack behind it as "Queued · Nth in line". Reachable from the
clear-context restart, where the chat is still `Working`; see the `TODO(bug)` note in
`plan-clear-context.spec.ts`.
