...
| Inline review → agent | `editor-inline-comment-send`, `editor-line-comment-send`, `editor-submit-review`, `line-comment-widget`                                                                                                                                            | `sendCommentMessage` dispatches to the chat; the _send_ path is AI-coupled (open/type/cancel is not — see §4) |
...
| `59-new-branch-dialog`   | `new-branch-dialog`, `new-branch-back`, `new-branch-cancel`, `new-branch-start-point-select`, `rename-branch-back`, `rename-branch-cancel`                                                                                                                                                                                                   | Open new-branch + rename-branch dialogs from the branch UI; exercise start-point select; dismiss. (Verify branch _rename_ doesn't use `window.prompt` — the tag rename did, see `50-tags`.)                                                                                                                                                                    |
...
Of the six e2e scenarios the PR #688 fix plan
(`docs/plans/2026-09-14-pr-688-review-fixes-plan.md`, Verification step 7) asks for, two remain
unreachable from this suite. Not "hard to set up" — the code path that produces the behavior is
...
The other four are covered: `tests-tauri/facade-protocol-partial.spec.ts` (no ghost bubble after
a retry), `tests-tauri/facade-reconnect-mid-stream.spec.ts` (a mid-turn reconnect resumes
running), `tests-tauri/facade-queued-prompt.spec.ts` (a queued prompt lands at the tail on its
dequeue), and `tests-tauri/plan-clear-context.spec.ts` (plan-mode clear context stays cleared).

### 5.1 Plan-mode clear context — CLOSED

Was blocked because `mock-cli` resolved no plan-mode handler. `MockPlanModeHandler`
(`mainframe-adapter-mock/src/plan_mode_handler.rs`) now implements the clear-context branch, so
`_mainframe.dev/transcript_cleared` really fires. Covered by
`tests-tauri/plan-clear-context.spec.ts`.

### 5.2 A prompt queued behind a running turn — CLOSED

Was blocked because `mock-cli` reported `supports_replay_ack() == false`, so nothing was ever
enrolled in `queuedRefs`. The mock now parks a uuid-carrying prompt behind the running turn and
synthesizes the dequeue ack. Covered by `tests-tauri/facade-queued-prompt.spec.ts`.

The residual first seen here (a uuid-carrying prompt replayed without an ack after the
clear-context restart) was a mock-fidelity gap: `mock-cli`'s `kill()` never reported `on_exit`,
so the chat stayed `Working` and the auto-sent prompt was enrolled in `queuedRefs`. The mock now
reports the exit and acks any uuid-carrying prompt it replays at once; `plan-clear-context.spec.ts`
asserts the auto-sent turn renders as an ordinary user bubble with no queued badge.
...
| Blocked — absent under `E2E_MODE=mock` | 2 scenarios | pinned by unit tests, see §5       |
