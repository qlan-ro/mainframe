...
import { collectDefinitions, collectReferences } from './extract.mjs';
...
/**
 * True when some definition matches the ref's prefix directly, or — when
 * either side is templated — when the definition's prefix extends the ref's
 ...
 * @returns {boolean}
 */
export function isLiveReference(defs, ref) {
...
function buildDeadAndPerSpec(definitions, strictBySpec) {
  ...
  for (const { spec, strict } of strictBySpec) {
    ...
    for (const ref of strict) {
      if (isLiveReference(definitions, ref)) {
        live += 1;
        continue;
      }
      dead += 1;
...
export function analyze({ sourceFiles, specFiles }) {
  ...
  const { dead, perSpec } = buildDeadAndPerSpec(definitions, strictBySpec);
