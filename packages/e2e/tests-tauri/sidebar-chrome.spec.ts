...
test.describe('§sidebar-chrome', () => {
  ...
  test('workflows button opens the workflows modal', async () => {
    ...
    // No run is selected on first open, so Escape hits the default Radix close path
    // (WorkflowsModalHost.tsx only intercepts onEscapeKeyDown when selectedRunId != null).
