...
export function MenuDivider({ className, section }: { className?: string; section?: boolean }) {
  return (
    <div
      className={cn(section ? 'border-t border-border' : 'mx-1.5 my-[4px] border-t-[0.5px] border-border', className)}
    />
  );
}
...
export function MenuEmpty({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div
      className={cn(
        'flex flex-col items-center justify-center gap-1.5 px-2 py-4 text-caption text-mf-text-3',
        className,
      )}
    >
      {children}
