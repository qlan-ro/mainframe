...
import { applyStepsEdit, applyVariableRenames } from '../definition-actions';
...
describe('applyVariableRenames', () => {
  it('rewrites downstream refs to the renamed value', () => {
    ...
    expect(messageOf(applyVariableRenames(previous, next, NO_CATALOG), 'n1')).toBe('Ship $title today');
  });
  ...
  it('leaves a longer name that merely starts with the old one alone', () => {
    ...
    expect(messageOf(applyVariableRenames(previous, next, NO_CATALOG), 'n1')).toBe('$title vs $headliner');
  });
  ...
  it('keeps the dotted path when the base name changes', () => {
    ...
    expect(messageOf(applyVariableRenames(previous, next, NO_CATALOG), 'n1')).toBe('See $shipment.notes');
  });
  ...
  it('returns the patch untouched when no name changed', () => {
    ...
    expect(applyVariableRenames(previous, next, NO_CATALOG).steps).toEqual(next.steps);
  });
  ...
  it('rewrites nothing when a previously unnamed value is named for the first time', () => {
    ...
    expect(messageOf(applyVariableRenames(previous, next, NO_CATALOG), 'n1')).toBe('Ship $headline');
  });
  ...
  it('rewrites refs held inside a block, from a rename made outside it', () => {
    ...
    const repeat = applyVariableRenames(previous, next, NO_CATALOG).steps[1];
  ...
  it('rewrites from a rename made inside a block — nesting hides no set-value step', () => {
    ...
    expect(messageOf(applyVariableRenames(previous, next, NO_CATALOG), 'n1')).toBe('Ship $title');
  });
  ...
  it('ignores a renamed ask_me field key — only set-value names rewrite refs (the stale $ref is a save-time error)', () => {
    ...
    expect(messageOf(applyVariableRenames(previous, next, NO_CATALOG), 'n1')).toBe('Ship $field_1');
  });
  ...
  // Renaming onto a taken name used to rewrite the *other* holder's refs onto
  // this step, silently. The refs stay put; `validate` reports the duplicate.
  it('rewrites nothing when the new name already belongs to a later step', () => {
    const message = 'Ship $title, filed under $headline';
    const previous = definition([setValue('v1', 'headline'), setValue('v2', 'title'), notify('n1', message)]);
    const next = definition([setValue('v1', 'title'), setValue('v2', 'title'), notify('n1', message)]);

    expect(messageOf(applyVariableRenames(previous, next, NO_CATALOG), 'n1')).toBe(message);
  });

  it('rewrites nothing when the new name belongs to a step in an isolated repeat body', () => {
    const body = (steps: AutomationStep[]): AutomationStep => ({
      id: 'r1',
      kind: 'repeat',
      items: { stepId: 'builtin', output: 'today' },
      steps,
    });
    const inner = setValue('v2', 'title');
    const previous = definition([setValue('v1', 'headline'), body([inner]), notify('n1', 'Ship $headline')]);
    const next = definition([setValue('v1', 'title'), body([inner]), notify('n1', 'Ship $headline')]);
...
describe('applyStepsEdit', () => {
  ...
  function outputNameOf(result: AutomationDefinition, id: string): string | undefined {
    const step = result.steps.find((s) => s.id === id);
    if (step?.kind !== 'ask_agent') throw new Error(`no agent step ${id}`);
    return step.outputName;
  }

  it('names a step the edit just added', () => {
    const previous = definition([]);

    const result = applyStepsEdit(previous, [askAgent('a1')], NO_CATALOG);

    expect(outputNameOf(result, 'a1')).toBe('agent_result');
  });

  it('leaves the incumbent bare when the new step is added above it', () => {
    const previous = definition([askAgent('a1', 'agent_result')]);

    const result = applyStepsEdit(previous, [askAgent('a0'), askAgent('a1', 'agent_result')], NO_CATALOG);

    expect(outputNameOf(result, 'a1')).toBe('agent_result');
    expect(outputNameOf(result, 'a0')).toBe('agent_result_2');
  });

  it('still applies the rename an edit carries', () => {
    const previous = definition([setValue('v1', 'headline'), notify('n1', 'Ship $headline')]);

    const result = applyStepsEdit(previous, [setValue('v1', 'title'), notify('n1', 'Ship $headline')], NO_CATALOG);

    expect(messageOf(result, 'n1')).toBe('Ship $title');
  });
});
