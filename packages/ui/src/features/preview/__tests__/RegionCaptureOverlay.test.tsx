...
describe('RegionCaptureOverlay', () => {
  it('renders overlay with testid', () => {
    const { getByTestId } = render(<RegionCaptureOverlay onRegionSelect={vi.fn()} onClose={vi.fn()} />);
    expect(getByTestId('preview-region-overlay')).toBeTruthy();
  });

  it('calls onRegionSelect with normalized rect on mouseup', () => {
    const onRegionSelect = vi.fn();
    const { getByTestId } = render(<RegionCaptureOverlay onRegionSelect={onRegionSelect} onClose={vi.fn()} />);
  ...
  it('calls onClose on Escape key', () => {
    const onClose = vi.fn();
    const { getByTestId } = render(<RegionCaptureOverlay onRegionSelect={vi.fn()} onClose={onClose} />);
    const overlay = getByTestId('preview-region-overlay');
  ...
  it('shows selection rect during drag', () => {
    const { getByTestId, queryByTestId } = render(<RegionCaptureOverlay onRegionSelect={vi.fn()} onClose={vi.fn()} />);
    const overlay = getByTestId('preview-region-overlay');
