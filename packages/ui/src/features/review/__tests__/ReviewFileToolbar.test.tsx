...
// ---------------------------------------------------------------------------
// Viewed-chip alpha values (design-exact, per verifier correction on 11.14)
// ---------------------------------------------------------------------------

describe('ReviewFileToolbar — viewed-chip alpha values', () => {
  it('uses the design border alpha (~33%, /[33%]) not /40 when viewed', () => {
    render(
      <ReviewFileToolbar
        file="src/components/Layout.tsx"
        additions={18}
    ...
    const toggle = screen.getByTestId('review-viewed-toggle');
    expect(toggle.className).toContain('border-mf-success/[33%]');
    expect(toggle.className).not.toContain('border-mf-success/40');
  });

  it('uses the design background alpha (~8.6%, /[8.6%]) not /10 when viewed', () => {
    render(
      <ReviewFileToolbar
        file="src/components/Layout.tsx"
        additions={18}
    ...
    const toggle = screen.getByTestId('review-viewed-toggle');
    expect(toggle.className).toContain('bg-mf-success/[8.6%]');
    expect(toggle.className).not.toContain('bg-mf-success/10');
  });
});
