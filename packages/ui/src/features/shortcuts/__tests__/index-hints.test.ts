...
function hold(modifier: 'ctrlKey' | 'altKey') {
  window.dispatchEvent(new KeyboardEvent('keydown', { [modifier]: true }));
}
function release(modifier: 'ctrlKey' | 'altKey') {
  window.dispatchEvent(new KeyboardEvent('keyup', { [modifier]: false }));
}

describe('hintModifierHeld', () => {
  it('reads ⌃ on macOS and Alt elsewhere', () => {
    expect(hintModifierHeld({ ctrlKey: true, altKey: false }, true)).toBe(true);
    expect(hintModifierHeld({ ctrlKey: false, altKey: true }, true)).toBe(false);
    expect(hintModifierHeld({ ctrlKey: false, altKey: true }, false)).toBe(true);
    expect(hintModifierHeld({ ctrlKey: true, altKey: false }, false)).toBe(false);
  });
});
...
describe('useIndexHintReveal', () => {
  beforeEach(() => {
    ...
    // jsdom's navigator reports a non-Mac platform, so the hint modifier is Alt.
